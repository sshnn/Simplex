...
def main():
    global decimals
    global const_num, prod_nums ## ürün ve kısıt sayisi
    print(""" SİMPLEX MAKZİMİZASYON """)   
    prob_type = 1
    print('\n##########################################')
    global const_names
    const_num = int(input("Kaç Ürün: >"))
    prod_nums = int(input("Kaç Kısıt: >"))
    const_names = [x + str(i) for i in range(1, const_num + 1)]
    for i in range(1, prod_nums + 1):
        prod_val = input("{}. kısıt ismini giriniz: >".format(i))
        product_names.append(prod_val) ## kısıt ismini atar
    print("__________________________________________________")
    
    for i in const_names:
        try:
            val = float(Fraction(input("Z denklemindeki %s katsayısı : >" % i)))
        except ValueError:
    ...
    for prod in product_names:
        for const in const_names:
            try:
                val = float(Fraction(input("%s'nın %s katsayısı: >" % (prod, const))))
            except ValueError:
                print("please ensure you enter a number")
                val = float(Fraction(input("%s'nın %s katsayısı: >" % (prod, const))))
            col_values.append(val)
        equate_prod = float(Fraction(input('%s değeri: >' % prod)))
        col_values.append(equate_prod)

    final_cols = stdz_rows(col_values)
    i = len(const_names) + 1
    while len(const_names) < len(final_cols[0]) - 1:
        const_names.append('S' + str(i - 2))
        solutions.append('S' + str(i - 2))
        i += 1
    solutions.append(' Z')
    const_names.append('Çözüm')
...
def maximization(final_cols, final_rows):
    ...
    while min_last_row < 0 < pivot_element != 1 and min_manager == 1 and count < 6:
        ...
        print('pivot sütun: ', pivot_row)
        print('pivot satır: ', pivot_col)
        print("\n")
